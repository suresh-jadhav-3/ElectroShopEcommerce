  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProduct(shuffledProducts.slice(0, 8));
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 items-center">
            {loading ? (
              ...
              product.map((value, index) => (
                <div
                  key={index}
                  className="h-auto bg-white flex flex-col rounded overflow-hidden shadow-md cursor-pointer hover:scale-[1.01] transition-all"
                >
