      ...
      try {
        ...
        const products = response.data.products
          ...
          .map((product) => ({
            ...
            img: product.thumbnail, // or product.images[0]
          }));
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full flex items-center justify-center">
        <div
          className={`w-full h-full transition-opacity duration-300 ${fade ? "opacity-100" : "opacity-0"}`}
        >
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
        {slides.map((_, index) => (
          <div
            key={index}
            className={`w-4 h-1 rounded ${index === currentSlide ? "bg-red-600" : "bg-white/50"}`}
          ></div>
        ))}
      </div>
    </div>
